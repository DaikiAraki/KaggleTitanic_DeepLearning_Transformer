...
class Model(ModelCore):
    ...
    def __init__(self, initValues=None, name="model"):
        ...
        """ model """
        # 入力をTransformer Blockに入れる為にチャネル次元を追加する
        self.ExpandDimLayer = ReshapingLayer(
            shape=[Ref.input_node, 1], initValues=initValues, name=n_ExpandDimLayer)
        # Transformer Blockをいくつか重ねる
        self.TransformerBlocks = []
        for i in range(Ref.transformerBlocks_num):
            self.TransformerBlocks.append(
                TransformerBlock(
                    inWidth=Ref.input_node if (i == 0) else self.TransformerBlocks[-1].outWidth,
                    inChl=1 if (i == 0) else self.TransformerBlocks[-1].outChl,
                    outChl=Ref.transformerBlocks_outChl[i],
                    qkvChl=Ref.transformerBlocks_qkvChl[i],
                    actFunc=mish,
                    headNum=Ref.transformerBlocks_head_num[i],
                    innerExpansionRatio=Ref.transformerBlocks_inner_expansion_ratio[i],
                    downsampling=Ref.transformerBlocks_downsampling[i],
                    use_firstNormalization=Ref.transformerBlocks_use_first_normalize[i],
                    use_secondNormalization=Ref.transformerBlocks_use_second_normalize[i],
        ...
        self.FlattenBlock = FlattenBlock(
            ...
            outNode=Ref.flattenBlock_outNode, actFunc=mish, use_bias=True,
            normalize=Ref.flattenBlock_normalize, pre_normalize=True,
        ...
        self.OutputBlock = FcBlock(
            ...
            normalize=False, pre_normalize=False, dropout=False, d_rate=False,
            initValues=initValues, name=n_OutputBlock)
        ...
        self._register_objects(
            obj_dict={**{n_ExpandDimLayer: self.ExpandDimLayer},
    ...
    def call(self, X, L):
        ...
        """
        h_expandDimLayer = self.ExpandDimLayer.call(x=X)  # [b,w,1]
        h_transformerBlocks = []
        for blk in range(len(self.TransformerBlocks)):
