...
"""
Adam (https://arxiv.org/abs/1412.6980) の実装である tf.keras.optimizers.Adam を拡張したクラス。
slotsの入出力が可能になっている。
...
class Adam(tf.keras.optimizers.Adam):
    ...
    def __init__(self, learning_rate=1.e-4, beta_1=0.9, beta_2=0.999, epsilon=1.e-7, amsgrad=False,
                 name="Adam", **kwargs):
        super(Adam, self).__init__(learning_rate=learning_rate, beta_1=beta_1, beta_2=beta_2, epsilon=epsilon,
                                   amsgrad=amsgrad, name=name, **kwargs)
    ...
    def add_slot(self, var, slot_name, initializer="zeros"):
        ...
        if weight is None:
            if isinstance(initializer, six.string_types) or callable(initializer):
                ...
                initial_value = functools.partial(initializer, shape=var.shape, dtype=var.dtype)
            else:
            ...
            if not strategy.extended.variable_created_in_scope(var):
                raise ValueError(
                    ...
                    "happen if you're restoring from a checkpoint outside the scope".format(strategy, var))
    ...
    @property
    def iterations(self):
        """Variable. The number of training_with_storage steps this Optimizer has run."""
